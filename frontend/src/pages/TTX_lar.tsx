...
const Ttxlar: React.FC = () => {
    ...
    const handleVehicleMouseEnter = (vehicleId: string) => {
    ...
    const handleVehicleMouseLeave = (e: React.MouseEvent) => {
        const relatedTarget = e.relatedTarget as HTMLElement;

        // Agar foydalanuvchi expandable sectionga o'tsa, hover ni yo'qotma
        if (relatedTarget?.closest?.(".expandable-section")) {
            return;
        }
    ...
    const handleExpandableMouseEnter = () => {
        if (timeoutRef.current) {
            clearTimeout(timeoutRef.current);
        }
    };

    const handleExpandableMouseLeave = (e: React.MouseEvent) => {
        const relatedTarget = e.relatedTarget as HTMLElement;

        // Agar foydalanuvchi asosiy kartaga o'tsa, hover ni saqla
        if (relatedTarget?.closest?.(".vehicle-card")) {
            return;
        }
    ...
    const handleDetailsClick = (e: React.MouseEvent, vehicle: Vehicle) => {
        e.stopPropagation();
                        ...
                        {filteredByType.length > 0 ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6">
                                {filteredByType.map((vehicle) => (
                                    <div
                                        key={vehicle.id}
                                        className={`relative group vehicle-card ${
                                            expandedVehicle === vehicle.id
                                                ? "z-20"
                                                : "z-0"
                                        }`}
                                        onMouseEnter={() =>
                                            handleVehicleMouseEnter(vehicle.id)
                                        }
                                        onMouseLeave={handleVehicleMouseLeave}
                                    >
                                        {/* Main Card */}
                                        <div className="bg-white/5 backdrop-blur-lg rounded-xl overflow-hidden transition-all duration-500 transform border border-white/10 hover:border-blue-400/30 flex flex-col h-full cursor-pointer">
                                            {/* Manufacturer Country Flag */}
                                            <div className="absolute top-4 right-4 z-10">
                                                <div className="w-10 h-7 overflow-hidden shadow-lg border border-white/20 bg-white">
                                            ...
                                            <div className="h-48 bg-gradient-to-br from-blue-500 to-blue-600 relative overflow-hidden flex-shrink-0">
                                                ...
                                                <div className="relative w-full h-full flex items-center justify-center p-4">
                                                    <img
                                                        src={vehicle.image}
                                                        alt={vehicle.name}
                                                        className="max-w-full max-h-full object-contain transition-transform duration-500 drop-shadow-lg"
                                                        onError={(e) => {
                                        ...
                                        <div
                                            className={`
                                                expandable-section absolute top-full left-0 right-0 mt-2
                                                bg-slate-800/95 backdrop-blur-md rounded-2xl border border-white/20
                                                overflow-hidden transition-all duration-500 ease-in-out shadow-2xl
                                                ${
                                                    expandedVehicle ===
                                                    vehicle.id
                                            ...
                                            `}
                                            onMouseEnter={
                                                handleExpandableMouseEnter
                                            }
                                            onMouseLeave={
                                                handleExpandableMouseLeave
                                            }
                                        >
                                            <div className="p-2">
                                                {/* Short Description */}
                                                ...
                                                <div className="mt-2 pt-2 border-t border-white/10">
                                                    <button
                                                        ...
                                                        className="w-full bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white py-2 px-3 rounded-lg transition-all duration-300 font-semibold flex items-center justify-center gap-2 text-sm"
                                                    >
                                                        Batafsil
                                                        <svg
